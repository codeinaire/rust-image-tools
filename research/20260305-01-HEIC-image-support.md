...
The only viable path to HEIC _decoding_ in a browser is using `libheif` (a C++ library) compiled
...
applications. HEIC _encoding_ in the browser is theoretically possible using kvazaar (LGPL) or
...
**Primary recommendation:** Add HEIC _reading_ only, using a hybrid architecture: handle HEIC
decode in JavaScript (via `libheif-js` or `heic-to` npm packages) before the WASM boundary, then
hand raw RGBA pixels into the existing Rust/WASM pipeline for re-encoding to any target format. Do not attempt to write HEIC output — the tooling is immature, the codec is GPL/patent-encumbered, and user demand for "convert to HEIC" is essentially zero.
...
HEIC is treated as an _input-only pre-processing step_ that produces RGBA pixels, which then feed
...
**How to avoid:** Restrict HEIC to _reading only_. For HEIC decode, libheif itself is LGPL, which
is generally safe for web distribution. `heic-to` is MIT-licensed. Verify with legal counsel before
shipping HEIC _encoding_ in a commercial product.
...
iPhone-driven, so Safari users are the _primary_ HEIC use case.
...
The key insight: the users _producing_ HEIC files (iPhone users) are disproportionately using
Safari, which _already_ supports HEIC natively. The remaining cases (Windows/Chrome users receiving
