...
function analyzeWasm() {
  ...
  try {
    ...
    const totalTracked =
      codeBytes + dataBytes + Object.values(customMap).reduce((s, v) => s + v, 0) + otherBytes

    const pct = (b) => (totalTracked > 0 ? ((b / totalTracked) * 100).toFixed(1) : '0.0')
    ...
    top3 = candidates.slice(0, 3).map((c) => cell(`${c.label}: ${fmt(c.bytes)} (${pct(c.bytes)}%)`))
